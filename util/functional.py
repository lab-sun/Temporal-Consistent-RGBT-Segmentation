...
def soft_homo_dice_score(
    output: torch.Tensor, target: torch.Tensor, output_homo: torch.Tensor, smooth: float = 0.0, eps: float = 1e-7, dims=None
) -> torch.Tensor:
    """
    ...
    """
    assert output.size() == target.size() == output_homo.size()
    if dims is not None:
        intersection = torch.sum(output * target * output_homo, dim=dims)
        cardinality = torch.sum(output + target + output_homo, dim=dims)
    else:
        intersection = torch.sum(output * target * output_homo)
        cardinality = torch.sum(output + target + output_homo)
    dice_score = (3.0 * intersection + smooth) / (cardinality + smooth).clamp_min(eps)
    return dice_score
