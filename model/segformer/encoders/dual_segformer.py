...
class RGBXTransformer(nn.Module):
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        # stage 1 
        x_rgb, H, W = self.patch_embed1(x_rgb)          # (n,3,512,640) to (n,20480,64)
        # B H*W/16 C
        x_e, _, _ = self.extra_patch_embed1(x_e)        # (n,3,512,640) to (n,20480,64)
        ...
        x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()         # (n,20480,64) to (n,64,128,160)
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        x_rgb, x_e = self.FRMs[0](x_rgb, x_e)                                       # (n,64,128,160)
        ...
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_rgb, x_e = self.FRMs[1](x_rgb, x_e)                                       # (n,128,64,80)
        ...
        x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_rgb, x_e = self.FRMs[2](x_rgb, x_e)                                       # (n,320,32,40)
        ...
        x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_rgb, x_e = self.FRMs[3](x_rgb, x_e)                                       # (n,512,16,20)
